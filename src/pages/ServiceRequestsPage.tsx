...
import type { ServiceRequestStatus, ServiceRequestPriority, ServiceRequestCategory } from '@/types';
...
export function ServiceRequestsPage() {
  const { user } = useAuth();
  const [requests, setRequests] = useState<ServiceRequestsResponse | null>(null);
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-gray-500 mt-1">
                      Request ID: {request.id.slice(0, 8)}... • {format(new Date(request.created_at), 'MMM d, yyyy h:mm a')}
                    </p>
                    {request.completed_at && (
                      <p className="text-sm text-green-600 mt-1">
                        Completed on {format(new Date(request.completed_at), 'MMM d, yyyy')}
                      </p>
                    )}
                  </div>
                  {isAdmin && (
