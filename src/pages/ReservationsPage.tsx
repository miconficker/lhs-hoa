...
import type { AmenityType, ReservationSlot, ReservationStatus, AmenityAvailability } from '@/types';
...
export function ReservationsPage() {
  const { user } = useAuth();
  const [myReservations, setMyReservations] = useState<ReservationsResponse | null>(null);
  ...
  async function loadData() {
    ...
    const availResult = await api.reservations.getAvailability(monthStart, monthEnd, bookingForm.amenity_type);
  ...
  async function handleCancelReservation(reservationId: string) {
    ...
    const result = await api.reservations.update(reservationId, { status: 'cancelled' });
    if (result.error) {
      setError(result.error);
    } else {
        ...
        {days.map(day => {
          ...
          return (
            <div
              ...
              onClick={() => !isPast && setBookingForm({ ...bookingForm, date: format(day, 'yyyy-MM-dd') })}
            >
              ...
              {!isPast && avail && (
                <div className="space-y-1">
                  <div className={`text-xs px-1 py-0.5 rounded ${avail.am_available ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                    AM: {avail.am_available ? 'Available' : 'Booked'}
                  </div>
                  <div className={`text-xs px-1 py-0.5 rounded ${avail.pm_available ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-900">Amenity Reservations</h1>
        <button
          onClick={() => setShowBookingForm(!showBookingForm)}
          className="flex items-center gap-2 px-4 py-2 bg-primary-600 text-white rounded-lg hover:bg-primary-700"
        >
          {showBookingForm ? <XMarkIcon className="w-5 h-5" /> : <PlusIcon className="w-5 h-5" />}
      ...
      {showBookingForm && (
        <div className="bg-white rounded-lg shadow p-6">
          <h2 className="text-lg font-semibold text-gray-900 mb-4">Book an Amenity</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setBookingForm({ ...bookingForm, amenity_type: e.target.value as AmenityType })}
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {(['AM', 'PM'] as ReservationSlot[]).map((slot) => {
                    const available = isSlotAvailable(parseISO(bookingForm.date), slot);
                    return (
                      <button
                        key={slot}
                        type="button"
      ...
      <div className="bg-white rounded-lg shadow p-6">
        ...
        {myReservations?.reservations && myReservations.reservations.length > 0 ? (
            ...
            {myReservations.reservations.map((reservation) => (
              ...
              >
                <div className="flex items-start gap-4">
                  <div className="text-4xl">{amenityIcons[reservation.amenity_type]}</div>
                  <div>
